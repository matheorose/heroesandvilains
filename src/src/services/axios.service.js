...
import {authHeader} from "@/services/org.service";


const baseURL = 'https://apidemo.iut-bm.univ-fcomte.fr'  // URL de base de l'API
 ...
 */
async function getRequest(uri, name,customConfig = {}) {
    let response = null
    try {
        response = await axiosAgent.get(uri,customConfig)
    } catch (err) {
...
async function postRequest(uri, data, name,customConfig = {}) {
    let response = null
    try {
        response = await axiosAgent.post(uri, data,customConfig)
    } catch (err) {
        // le catch se fait si le serveur répond avec une erreur type 4XX, 5XX, ou bien si le serveur est off
...
async function deleteRequest(uri, name,customConfig = {}) {
    let response = null
    try {
        response = await axiosAgent.delete(uri,customConfig)
    } catch (err) {
        // le catch se fait si le serveur répond avec une erreur type 4XX, 5XX, ou bien si le serveur est off
        // dans ce cas, on appelle la méthode pour traiter ces types d'erreurs
        response = handleError(name, err);
...
async function patchRequest(uri, data, name,customConfig = {}) {
    let response = null
    try {
        response = await axiosAgent.patch(uri, data,customConfig)
    } catch (err) {
        // le catch se fait si le serveur répond avec une erreur type 4XX, 5XX, ou bien si le serveur est off
        // dans ce cas, on appelle la méthode pour traiter ces types d'erreurs
        response = handleError(name, err);
...
async function putRequest(uri, data, name,customConfig = {}){
    let response = null
    try {
        response = await axiosAgent.put(uri, data,customConfig)
    } catch (err) {
        // le catch se fait si le serveur répond avec une erreur type 4XX, 5XX, ou bien si le serveur est off
        // dans ce cas, on appelle la méthode pour traiter ces types d'erreurs
        response = handleError(name, err);
